export async function sendToInvoiceXpressAutomation(metadata, stripeSessionId) {
  ...
  const payload = {
    event: "soundbunker.booking.deposit_paid",
    ...
    invoice: { currency: "EUR", vat_rate: 23, total_including_vat: Number(metadata.total), payment_received_including_vat: Number(metadata.deposit), balance_due_including_vat: balance, due_date: metadata.date },
