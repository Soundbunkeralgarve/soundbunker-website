...
export async function createCheckoutSession({ session, booking, origin, bookingRef }) {
  ...
  const metadata = {
    ...
    no_slot: session.noSlot ? "true" : "false",
    full_payment: session.fullPayment ? "true" : "false"
  };
  const body = new URLSearchParams({
    ...
    "line_items[0][price_data][product_data][description]": session.fullPayment ? "VAT included. Payment in full." : `${booking.date} at ${booking.time}. VAT included. Balance due later.`
  });
