...
const isCrudeCity = document.body.dataset.store === 'crude-city';
const basketKey = isCrudeCity ? 'cc-shop-basket' : 'sb-shop-basket';
let basket = [], quote = null, revision = 0, busy = false, next = 0, selectedCategory = 'all', selectedCollection = isCrudeCity ? 'crude-city' : 'all', adultConfirmed = false, catalogEpoch = 0;
...
function filterProducts() {
 ...
 document.querySelectorAll('.shop-card').forEach(card=>{
  card.hidden=(isCrudeCity ? (card.dataset.collection!=='crude-city'||!adultConfirmed) : card.dataset.collection==='crude-city')||(selectedCollection!=='all'&&card.dataset.collection!==selectedCollection)||(category!=='all'&&card.dataset.category!==category)||(color!=='all'&&!card.dataset.colors.split(/[|/]/).map(c=>c.trim()).includes(color))||(query&&!card.dataset.search.includes(query));
...
try { const saved=JSON.parse(localStorage.getItem(basketKey) || '[]'); if(Array.isArray(saved)) basket=saved.filter(i=>Number.isSafeInteger(i.id)&&Number.isInteger(i.quantity)&&i.quantity>0&&i.quantity<=10&&typeof i.name==='string'&&Number.isSafeInteger(i.price)).slice(0,15); } catch {}
const el = (tag,text,cls) => {const n=document.createElement(tag);if(text!==undefined)n.textContent=text;if(cls)n.className=cls;return n;};
async function api(action,body,params='') { const r=await fetch(`/api/shop?action=${action}${params}`,{method:body?'POST':'GET',headers:{...(body?{'content-type':'application/json'}:{}),'x-sb-shop-brand':isCrudeCity?'crude-city':'soundbunker',...(adultConfirmed?{'x-sb-adult-confirmed':'true'}:{})},...(body?{body:JSON.stringify(body)}:{})});const d=await r.json();if(!r.ok)throw Error((d.error||'Please try again.')+(d.code?` (Reference: ${d.code}${d.provider_status?'-'+d.provider_status:''})`:''));return d; }
...
function save(){try{localStorage.setItem(basketKey,JSON.stringify(basket));}catch{}invalidate();renderBasket();}
...
function selectCollection(key){
 if(isCrudeCity&&key!=='crude-city')return;
...
document.getElementById('age-confirm').onclick=()=>{adultConfirmed=true;document.body.classList.add('age-cleared');catalogEpoch++;document.getElementById('age-gate').close();next=0;selectCollection('crude-city');renderBasket();loadProducts();};
document.getElementById('age-cancel').onclick=()=>{if(isCrudeCity)location.assign('https://www.soundbunker.pt/');else document.getElementById('age-gate').close();};
const initialCollection=new URLSearchParams(location.search).get('collection');
if(!isCrudeCity&&Object.hasOwn(collectionNames,initialCollection||'')&&initialCollection!=='crude-city')selectCollection(initialCollection);
renderBasket();if(isCrudeCity)document.getElementById('age-gate').showModal();else loadProducts();
...
function shareProductButton(product) {
 ...
 button.onclick=async()=>{
  const url=new URL(isCrudeCity?(location.hostname.replace(/^www\./,'')==='crude-city.com'?'/':'crude-city.html'):'shop.html',location.href);if(!isCrudeCity)url.searchParams.set('collection',product.collection||'all');url.hash=product.id==='couples-combo'?'couples-combo':'product-'+product.id;
  const title=product.collection==='crude-city'?'Crude City · 18+':displayName(product);
  try{if(navigator.share){await navigator.share({title,text:title+(isCrudeCity?' · Crude City':' · SoundBunker'),url:url.href});return;}await navigator.clipboard.writeText(url.href);button.textContent='Link copied';}
