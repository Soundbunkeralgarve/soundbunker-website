...
if(!mixed.length)return;track.replaceChildren();for(const p of mixed.slice(0,5)){const a=document.createElement('a');a.className='home-merch-card';a.href='shop.html#product-'+p.id;const img=document.createElement('img');img.src=p.image;if(p.campaign)img.className='campaign-image';img.alt=p.name;img.width=400;img.height=500;img.loading='lazy';img.decoding='async';const h=document.createElement('h3');h.textContent=p.display_name||p.name;a.append(img,h);if(Number.isSafeInteger(p.price)){const price=document.createElement('p');price.textContent=new Intl.NumberFormat('en-IE',{style:'currency',currency:'EUR'}).format(p.price/100)+' · VAT included';a.append(price);}track.append(a);}}catch{/* Keep the visible shop link if the provider is unavailable. */}};
